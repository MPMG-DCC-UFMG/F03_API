...
class Item(BaseModel):
    ...
    grupo_id: str = Field(example="2122", title='ID do grupo')
    licitacao_item: str = Field(example="10019442", title='ID do item na licitação')
    regiao: str = Field(example="Zona da Mata", title='Região da Licitação')
    forma_pagamento: str = Field(example="15 DIAS", title='Forma de pagamento dos itens licitados')
    nome_homologado: str = Field(example="Loja do Fulano", title='Nome do licitante')
    num_cpf_cnpj_homologado : str = Field(example="12345678900", title='Documento do licitante')
    modalidade: str = Field(example="PREGAO PRESENCIAL", title='Modalidade da licitação')
    vlr_unitario: float = Field(example=12.99, title="Valor do item licitado")
    vlr_unitario_cotado: float = Field(example=12.99, title="Valor do item cotado")
    qtde_item_homologado: int = Field(example=30, title='Quantidade de itens homologados')
